...
class UploadDocument(Resource):
    ...
    def post(self):
        ...
        document_folder = os.path.join(UPLOAD_FOLDER, inspection_type, hashid, doc_type)
...
class DeleteDocument(Resource):
    ...
    @auth.login_required
    def delete_inspection_sms_form(self, inspection_type, oid):
        id_person_role = self.person_class.getPersonRoleId(g.user)
        id_user = str(id_person_role[0][0][0])
        # id_user = 100
        updated_date = datetime.datetime.now().strftime('%Y-%m-%d')
        if inspection_type == 'specific':
            SQL = "UPDATE cide_specific_inspection SET last_update=%s, id_user=%s, sms_form = NULL WHERE id_specific_inspection = %s RETURNING id_specific_inspection"
        else:
            SQL = "UPDATE cide_coordinated_inspection SET last_update=%s, id_user=%s, sms_form = NULL WHERE id_coordinated_inspection = %s RETURNING id_coordinated_inspection"
        self.connection.connect()
        update_data = self.connection.query(sql=SQL, data=(updated_date, id_user, oid), fetch=False)
        self.connection.close()
        return update_data
    ...
    @auth.login_required
    def delete_inspection_minutes(self, inspection_type, oid):
        id_person_role = self.person_class.getPersonRoleId(g.user)
        id_user = str(id_person_role[0][0][0])
        # id_user = 100
        updated_date = datetime.datetime.now().strftime('%Y-%m-%d')
        if inspection_type == 'specific':
            SQL = "UPDATE cide_specific_inspection SET last_update=%s, id_user=%s, minutes = NULL WHERE id_specific_inspection = %s RETURNING id_specific_inspection"
        else:
            SQL = "UPDATE cide_coordinated_inspection SET last_update=%s, id_user=%s, minutes = NULL WHERE id_coordinated_inspection = %s RETURNING id_coordinated_inspection"
        self.connection.connect()
        update_data = self.connection.query(sql=SQL, data=(updated_date, id_user, oid), fetch=False)
        self.connection.close()
        return update_data
    ...
    def delete(self):
        ...
        inspection_oid = (hashids.decode(inspection_hash))[0]
        if doc_type == 'final_report':
            delete_document = self.delete_inspection_final_report(inspection_type, inspection_oid)
        elif doc_type == 'sms_form':
            delete_document = self.delete_inspection_sms_form(inspection_type, inspection_oid)
        elif doc_type == 'minutes':
            delete_document = self.delete_inspection_minutes(inspection_type, inspection_oid)
        else:
            return Response(status=400)
        if delete_document:
            result = '{"updated":"' + hashids.encode(delete_document[0][0]) + '"}'
        else:
            result = '{"updated": 0}'
        return Response(result, mimetype='application/json')
